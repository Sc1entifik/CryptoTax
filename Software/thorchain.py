...
class ThorChain(Blockchain):
    viewblock_url = "https://viewblock.io/thorchain/address/"
    transactions_filepath = f"{Blockchain.blockchain_transactions_filepath}Thor/"
    ...
    def _datarows_list_from_tx_object(self, data_object):
        ...
        #fees column needs to be split differently because of an annoying semicoln which is not always present but ruins everything when it is.
    ...
    def download_thor_transactions(self):
        ...
        for wallet in self.wallet_address_dictionary.values():
            tx_report_url = f"{ThorChain.viewblock_url}{wallet}?network=mainnet"
            thorchain_transactions_filepath = f"{ThorChain.transactions_filepath}{wallet}.csv"
        
        self.dl_tx_report_link(tx_report_url, thorchain_transactions_filepath) 

        return f"Your default webbrowser has been loaded to {tx_report_url}. Complete the robot tests there and then save your transactions file to {thorchain_transactions_filepath}."
    ...
    def format_thor_transactions(self):
        ...
        for file in Path(ThorChain.transactions_filepath).glob("*.csv"):
                    
            with open(file) as transactions:
            ...
            with open(file, "w") as formatted_transactions:
        ...
        return "ThorChain transaction files have been modified! Please check for accuracy." 
